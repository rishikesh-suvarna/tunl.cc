...
describe('buildHttpResponse', () => {
  describe('headers', () => {
    ...
    it('does NOT inject default Content-Type when headers omit it', () => {
      const result = buildHttpResponse(200, { 'x-foo': 'bar' });
  ...
  describe('body', () => {
    ...
    it('decodes base64 body to a Buffer when bodyEncoding is base64', () => {
      ...
      const result = buildHttpResponse(
        200,
        undefined,
      ...
      expect((result.body as Buffer).equals(original)).toBe(true);
    });
    ...
    it('falls back to raw string when bodyEncoding is missing (legacy clients)', () => {
      const result = buildHttpResponse(200, undefined, 'hello world');
  ...
  describe('statusCode', () => {
    it('passes through 2xx/3xx/4xx/5xx', () => {
      expect(buildHttpResponse(204).statusCode).toBe(204);
      expect(buildHttpResponse(301).statusCode).toBe(301);
      expect(buildHttpResponse(404).statusCode).toBe(404);
      expect(buildHttpResponse(503).statusCode).toBe(503);
    });

    it('defaults to 200 when statusCode is 0/falsy', () => {
      expect(buildHttpResponse(0).statusCode).toBe(200);
    });
  });
});
...
describe('bodyByteLength', () => {
  it('returns Buffer length for buffers', () => {
    expect(bodyByteLength(Buffer.from('abc'))).toBe(3);
    expect(bodyByteLength(Buffer.from([0xff, 0xff, 0xff]))).toBe(3);
  });

  it('returns UTF-8 byte length for strings (multi-byte aware)', () => {
    expect(bodyByteLength('abc')).toBe(3);
    expect(bodyByteLength('ümlaut')).toBe(7);
